...
👨‍💻 AUTHOR & CREDITS:
   ...
   • Inspired by: ProofixAI (https://github.c• 𝟖 𝐀𝐈 𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠 𝐌𝐨𝐝𝐞𝐬 -om/geek-updates/proofixai)
...
class HttpRequestManager {
    ...
    static Request(method, url, data := "", headers := "", timeout := 10000, binary := false) {
        ...
        if (!InStr(url, "https://")) {
            LogWarn("Non-HTTPS URL detected: " . url)
            ...
            if (!InStr(url, "localhost") && !InStr(url, "127.0.0.1") && !InStr(url, "raw.githubusercontent.com")) {
                throw ValueError("HTTPS required for security", A_ThisFunc)
            }
        }
...
ProofreadText(inputText, apikey, mode := "fix") {
    ...
    try {
        ...
        ; Validate response
        if (!response || !response.Has("success") || !response["success"]) {
            errorMsg := response.Has("error") ? response["error"] : "Unknown HTTP error"
            throw OSError("HTTP Request failed: " . errorMsg, A_ThisFunc)
        }
        
        responseText := response.Has("body") ? response["body"] : ""
        if (!responseText) {
